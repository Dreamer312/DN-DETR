...
def prepare_for_dn(dn_args, embedweight, batch_size, training, num_queries, num_classes, hidden_dim, label_enc):
    ...
    # self.label_enc是#[152, 255]的Embedding层，这里num_classes是151，说明将Embedding最后一个元素拿出来，复制300次
    ...
    if training:
        ...
        known_indice = torch.nonzero(unmask_label + unmask_bbox) #unmask_label + unmask_bbox是52个2  [52, 1]
        known_indice = known_indice.view(-1) #[52]     0到51
        # add noise
        known_indice = known_indice.repeat(scalar, 1).view(-1) # 5*52=260
        known_labels = labels.repeat(scalar, 1).view(-1)  #[260]
        known_bid = batch_idx.repeat(scalar, 1).view(-1)  #[260]
        known_bboxs = boxes.repeat(scalar, 1)  #[260,4]
